...
int main(int argc, char* argv[]) {
	...
	for (string line; getline(in, line);) {		//parse through the file until the end
		...
		else if (item1 == "tree") {		//if tree is called, check for which mode we are in and then create the respective iterator object
			if (intVal == true) {
				if (bstInt.sizeTree() == 0) out << " empty";
				BST<int>::Iterator iter = bstInt.begin();
				out << " ";
				while (!iter.isEnd()) {			//iterate through our int stack outputting each item
					out << iter.curr()->data_ << " ";
					iter.next();
				}
			}
			else {
				if (bstString.sizeTree() == 0) out << " empty";
